...
impl DIDMethod for PkhMethod {
    ...
    fn address_for_message(&self, did: &str) -> Result<String, CryptoError> {
        ...
        let suite =
            find_cipher_suite(ns).ok_or_else(|| CryptoError::UnsupportedMethod(ns.to_string()))?;
        let (address, _) = suite.parse_did_parts(remainder)?;
        // For eip155 always return EIP-55 checksummed form.
        if ns == "eip155" {
            return crate::did::checksummed_address(did);
        }
        Ok(address)
    }
...
fn split_namespace(did: &str) -> Result<(&str, &str), CryptoError> {
    let rest = did
        .strip_prefix("did:pkh:")
        .ok_or_else(|| CryptoError::InvalidDid(format!("expected did:pkh: prefix: {did}")))?;
    let colon = rest
        .find(':')
        .ok_or_else(|| CryptoError::InvalidDid(format!("no namespace in: {did}")))?;
...
mod tests {
    ...
    fn verify_unknown_namespace_errors() {
        ...
        assert!(matches!(result, Err(CryptoError::UnsupportedMethod(_))));
    }
